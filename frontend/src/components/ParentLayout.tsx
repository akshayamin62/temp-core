...
export default function ParentLayout({ children, user }: ParentLayoutProps) {
      ...
      >
        ...
        <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
          {navigationItems.map((item) => {
            const isActive = pathname === item.path || (item.path !== '/parent/dashboard' && pathname.startsWith(item.path));
            return (
              <button
                key={item.path}
