...
export default function ReferrerLayout({ children, user }: ReferrerLayoutProps) {
      ...
      >
        ...
        <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
          {navigationItems.map((item) => {
            const isActive = pathname === item.path || (item.path !== '/referrer/dashboard' && pathname.startsWith(item.path));
            return (
              <button
                key={item.path}
