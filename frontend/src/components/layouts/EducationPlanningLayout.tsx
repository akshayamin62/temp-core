...
interface EducationPlanningLayoutProps {
  ...
  formStructure?: { part: { key: string; title: string; description?: string; order: number }; order: number; sections: any[] }[];
...
const eduPlanItems = [
  { key: 'dashboard', label: 'Dashboard' as const, icon: Icon.dashboard },
  { key: 'my-activity', label: 'My Activity' as const, icon: Icon.activity },
  { key: 'analytics', label: 'Activity Analysis' as const, icon: Icon.analytics },
  { key: 'brainography', label: 'Brainography Analysis' as const, icon: Icon.brainography },
  { key: 'portfolio', label: 'Portfolio Generator' as const, icon: Icon.portfolio },
];
...
export default function EducationPlanningLayout({
  children,
  user,
  serviceName = 'Education Planning',
  activeView,
  onViewChange,
  onMyActivityClick,
  formStructure = [],
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-white border-r border-gray-200 transition-all duration-300 flex flex-col sticky top-20 h-[calc(100vh-5rem)]`}>
        ...
        <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
          ...
          {eduPlanItems.map(item =>
            navBtn(item.key, item.label, item.icon, activeView === item.key, () => {
          ...
          {formStructure.map((part, i) =>
            navBtn(part.part.key, part.part.title, getPartIcon(part.part.title),
              currentPartIndex === i && activeView === 'form',
