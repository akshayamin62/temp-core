...
export default function B2BSalesLayout({ children, user }: B2BSalesLayoutProps) {
      ...
      >
        <div className="h-16 border-b border-gray-200 flex items-center justify-between px-4">
          {sidebarOpen && (
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-gray-900">B2B Sales</span>
            </div>
          )}
          <button
        ...
        <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
          {navigationItems.map((item) => {
            const isActive = pathname === item.path || (item.path !== '/b2b-sales/dashboard' && pathname.startsWith(item.path));
            return (
              <button
                key={item.path}
