...
export default function OpsParentsPage() {
                  ...
                  {filteredParents.length > 0 ? filteredParents.map((p) => (
                    <tr key={p._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <button onClick={() => router.push(`/ops/parents/${p._id}`)} className="px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium text-xs">View Detail</button>
                      </td>
                    </tr>
                  )) : (
