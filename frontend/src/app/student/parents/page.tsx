              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Relationship</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredParents.length > 0 ? filteredParents.map((p) => (
                    <tr key={p._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{p.relationship || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <button onClick={() => router.push(`/student/parents/${p._id}`)} className="px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium text-xs">View Detail</button>
                      </td>
                    </tr>
                  )) : (
                    <tr><td colSpan={6} className="px-6 py-12 text-center">
                      ...
                      <p className="text-sm text-gray-500">{searchQuery ? 'Try adjusting your search' : 'Parents will appear here once linked.'}</p>
                    </td></tr>
                  )}
                </tbody>
              </table>
