...
export default function AdminOnboardingPage() {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (userData.role !== USER_ROLE.ADMIN) { router.push('/'); return; }
      if (res.data.data.admin?.isVerified) { router.replace('/admin/dashboard'); return; }
      setUser(userData);
  ...
  const handleAddDocField = async (data: { documentName: string; required: boolean; helpText: string }) => {
    try {
      await b2bLeadDocumentAPI.addMyField(data);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-30">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-gray-900">Admin Onboarding</h1>
