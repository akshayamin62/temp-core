...
export default function AdvisorEducationPlanningPricingPage() {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (userData.role !== USER_ROLE.ADVISOR) { toast.error('Access denied.'); router.push('/'); return; }
      setUser(userData);
      await fetchPricing();
    ...
    <AdvisorLayout user={user}>
      ...
      <div className="bg-gradient-to-b from-slate-50 via-white to-slate-50 min-h-[calc(100vh-5rem)]">
        <div className="px-6 lg:px-8 py-8">
          <button onClick={() => router.push('/advisor/service-pricing')} className="mb-4 inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900 transition-colors font-medium">
