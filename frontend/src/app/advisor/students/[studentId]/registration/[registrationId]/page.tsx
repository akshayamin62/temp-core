...
import { authAPI, advisorAPI, programAPI, teamMeetAPI, opsScheduleAPI, activityAPI } from '@/lib/api';
...
export default function AdvisorStudentFormViewPage() {
  ...
  const fetchAllData = async () => {
    try {
      const studentResponse = await advisorAPI.getStudentDetail(studentId);
      const studentData = studentResponse.data.data.student;
      setStudentInfo(studentData);

      const registrationResponse = await advisorAPI.getStudentFormAnswers(studentId, registrationId);
        ...
        <div className="p-8">
          ...
          {studentInfo && serviceInfo && (
            <StudentFormHeader
              ...
              adminId={studentInfo.adminId?._id || studentInfo.advisorId?._id}
            />
          )}
          ...
          {isEducationPlanning && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-6 overflow-hidden">
              <div className="flex border-b border-gray-200">
                ...
                <button onClick={() => router.push(`/advisor/students/${studentId}/registration/${registrationId}/activity`)}
