...
import { authAPI, referrerAPI, programAPI, teamMeetAPI, opsScheduleAPI, activityAPI } from '@/lib/api';
import { User, USER_ROLE, TeamMeet, OpsSchedule, FormStructure, FormPartKey } from '@/types';
...
export default function ReferrerStudentRegistrationPage() {
  ...
  // Education Planning fetch functions
  ...
  const fetchBrainography = async () => {
    try {
      ...
      const response = await axios.get(`${BRAINOGRAPHY_API_URL}/brainography/${registrationId}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleBrainographyDownload = async () => {
    ...
    try {
      ...
      link.href = url; link.setAttribute('download', brainographyDoc.fileName);
      document.body.appendChild(link); link.click(); link.remove();
      window.URL.revokeObjectURL(url);
    } catch { toast.error('Failed to download brainography report'); }
  };

  const fetchBrainographyData = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${BRAINOGRAPHY_API_URL}/portfolio/${registrationId}/data`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const fetchPortfolios = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${BRAINOGRAPHY_API_URL}/portfolio/${registrationId}/portfolios`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const fetchAllData = async () => {
    try {
      const studentResponse = await referrerAPI.getStudentDetail(studentId);
      const studentData = studentResponse.data.data.student;
      setStudentInfo(studentData);

      const registrationResponse = await referrerAPI.getStudentFormAnswers(studentId, registrationId);
    ...
    <>
      ...
      <ReferrerLayout user={user}>
        <div className="p-8">
          ...
          {/* Read Only Badge */}
          {!isEducationPlanning && (
            <div className="mb-4 inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-800 rounded-lg border border-blue-200 text-sm font-medium">
          ...
          {/* Study Abroad Dashboard (read-only) */}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                    {dashboardStatCards.map((card) => {
                      ...
                      return (
                        <div key={card.title} className="bg-white rounded-xl shadow-sm border-2 border-gray-200 p-5">
                          <div className="flex items-center justify-between">
                ...
                <div className="mb-6">
                  {currentPart.key === 'APPLICATION' && (currentSection.title === 'Apply to Program' || currentSection.title === 'Applied Program') ? (
                    <div className="bg-white rounded-lg border border-gray-200 p-6">
                      ...
                      <ProgramSection studentId={studentId} sectionType={currentSection.title === 'Apply to Program' ? 'available' : 'applied'} registrationId={registrationId} userRole="REFERRER" isReadOnly={true} />
                    </div>
                  ) : (
                    <FormSectionRenderer
