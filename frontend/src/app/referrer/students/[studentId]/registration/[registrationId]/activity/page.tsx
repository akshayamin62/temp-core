...
/* ═══════════════════════════════════════════
   MAIN COMPONENT (READ-ONLY – REFERRER VIEW)
   ═══════════════════════════════════════════ */
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50/30 p-4 md:p-6 animate-fadeIn">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push(`/referrer/students/${studentId}/registration/${registrationId}`)}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-xs font-semibold border border-blue-200">👁 Read Only</span>
                ...
                <div className="absolute right-0 top-full mt-2 w-[420px] bg-white rounded-xl shadow-2xl border border-gray-200 z-50 animate-fadeIn overflow-hidden">
                  <div className="px-3 py-2.5 border-b border-gray-100 bg-gray-50">
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => handleCalNav('PREV')} className="w-7 h-7 flex items-center justify-center text-gray-600 bg-white border border-gray-200 rounded hover:bg-gray-50 text-sm font-bold">‹</button>
                        <span className="text-sm font-bold text-gray-800 min-w-[120px] text-center">{MONTHS[getMonth(calDate)]} {getYear(calDate)}</span>
                        <button onClick={() => handleCalNav('NEXT')} className="w-7 h-7 flex items-center justify-center text-gray-600 bg-white border border-gray-200 rounded hover:bg-gray-50 text-sm font-bold">›</button>
                      </div>
                      <div className="flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-1.5">
                      <select value={getMonth(calDate)} onChange={(e) => handleMonthChange(Number(e.target.value))} className="flex-1 px-1.5 py-0.5 text-xs border border-gray-200 rounded bg-white">
                        {MONTHS.map((m, i) => <option key={m} value={i}>{m}</option>)}
                      </select>
                      <select value={getYear(calDate)} onChange={(e) => handleYearChange(Number(e.target.value))} className="w-16 px-1.5 py-0.5 text-xs border border-gray-200 rounded bg-white">
                  ...
                  <div className="p-2" style={{ height: calView === 'month' ? 340 : 380 }}>
                    <Calendar localizer={localizer} events={calEvents} startAccessor="start" endAccessor="end" view={calView} date={calDate}
                      ...
                      selectable eventPropGetter={eventStyleGetter}
                      views={['month', 'week', 'day']} defaultView="month" popup
                      style={{ height: '100%' }} toolbar={false}
                      formats={{
                        timeGutterFormat: (date: Date) => format(date, 'HH:mm'),
                        eventTimeRangeFormat: ({ start: s, end: e }: { start: Date; end: Date }) => `${format(s, 'HH:mm')} - ${format(e, 'HH:mm')}`,
                      }}
                    />
                  </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <section className="lg:w-[40%] lg:sticky lg:top-4 lg:self-start">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <button onClick={() => setFocusOpen(!focusOpen)} className="w-full px-4 py-3 flex items-center justify-between bg-blue-600 hover:bg-blue-700 transition-colors">
                <div className="flex items-center gap-2">
                  ...
                  {focusLoaded ? (
                    <>
                      ...
                      <div className="mt-4 pt-3 border-t border-gray-100">
                        <h4 className="text-xs font-bold text-gray-600 uppercase tracking-wide mb-2 flex items-center gap-1.5"><span>💬</span> Monthly Feedback</h4>
          ...
          <section className="lg:w-[60%]">
            {selectedDate ? (
                  ...
                  <div className="flex border-b border-gray-100 overflow-x-auto scrollbar-none">
                    {TABS.map((tab, idx) => (
                      <button key={idx} onClick={() => setActiveTab(idx)}
                        className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold whitespace-nowrap transition-all border-b-2 ${activeTab === idx ? 'border-blue-500 text-blue-700 bg-blue-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}>
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            {[{ key: 'goalAcademic', label: 'Academic', icon: '📚' }, { key: 'goalNonAcademic', label: 'Non-Academic', icon: '🎨' }, { key: 'goalHabitFocus', label: 'Habit Focus', icon: '🔄' }, { key: 'goalPsychological', label: 'Psychological', icon: '🧠' }, { key: 'goalPhysical', label: 'Physical', icon: '💪' }, { key: 'goalReadingBooks', label: 'Reading Books', icon: '📖' }].map(({ key, label, icon }) => (
                              <div key={key} className="rounded-lg border border-gray-200 p-3">
                                <label className="flex items-center gap-1 text-xs font-semibold text-gray-500 mb-1.5"><span>{icon}</span>{label}</label>
                                <p className="text-sm text-gray-700 whitespace-pre-wrap min-h-[2rem]">{(planner as unknown as Record<string, string | number>)[key] as string || <span className="text-gray-300 italic">—</span>}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                    ...
                    {activeTab === 1 && (
                      <div className="space-y-4">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          ...
                          <div className="flex flex-wrap gap-1">{TYPE_LEGEND.map((t) => (<span key={t} className="bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded text-[10px]">{t}</span>))}</div>
                        </div>
                        {planner.plans.map((session) => {
                          ...
                          return (
                            <div key={session.session} className="rounded-lg border border-gray-200 overflow-hidden">
                              <div className="bg-gray-50 px-4 py-2 flex items-center gap-2 border-b border-gray-200"><span>{SESSION_ICONS[session.session]}</span><span className="text-sm font-semibold text-gray-700">{session.session}</span></div>
                              <div className="p-3 space-y-1.5">
                                {fixed?.before && (<div className="flex items-center gap-2 px-2 py-1.5 bg-gray-50 rounded"><span className="text-xs text-gray-500 font-semibold uppercase w-20">{fixed.before}</span><span className="px-2 py-1 border border-gray-200 rounded text-xs bg-gray-50 text-gray-600">{(planner as unknown as Record<string, string>)[FIXED_TIME_KEYS[fixed.before] as string] || '—'}</span></div>)}
                                {session.rows.map((row, rIdx) => (
                                  <div key={rIdx} className="flex items-start gap-1.5">
                                    <span className="w-[110px] px-2 py-1.5 border border-gray-200 rounded text-xs bg-gray-50 text-gray-600 shrink-0 mt-0.5">{row.time || '—'}</span>
                                    <span className="flex-1 px-2 py-1.5 border border-gray-200 rounded text-xs bg-gray-50 text-gray-600 break-words min-w-0">{row.activity || '—'}</span>
                                    <span className="w-16 px-1 py-1.5 border border-gray-200 rounded text-xs bg-gray-50 text-gray-600 text-center shrink-0 mt-0.5">{row.type || '—'}</span>
                                  </div>
                                ))}
                                {fixed?.after && (<div className="flex items-center gap-2 px-2 py-1.5 bg-gray-50 rounded"><span className="text-xs text-gray-500 font-semibold uppercase w-20">{fixed.after}</span><span className="px-2 py-1 border border-gray-200 rounded text-xs bg-gray-50 text-gray-600">{(planner as unknown as Record<string, string>)[FIXED_TIME_KEYS[fixed.after] as string] || '—'}</span></div>)}
                              </div>
                            </div>
                          );
                        })}
                    ...
                    {activeTab === 2 && (
                      <div className="space-y-4">
                        ...
                        {DOMAINS.map((domainName) => {
                          ...
                          planner.plans.forEach((session, sIdx) => { session.rows.forEach((row, rIdx) => { if (row.activity.trim() && row.type && typeToDomain(row.type) === domainName) domainRows.push({ sIdx, rIdx, row }); }); });
                          if (domainRows.length === 0) return null;
                          return (
                            <div key={domainName} className="rounded-lg border border-gray-200 overflow-hidden">
                              <div className="px-4 py-2.5 bg-gray-50 border-b border-gray-200 flex items-center justify-between"><span className="font-semibold text-gray-700 text-sm uppercase tracking-wide">{domainName}</span><span className="text-xs text-gray-400">{domainRows.filter(d => d.row.completed === 'Completed').length}/{domainRows.length} completed</span></div>
                              <div className="p-3 space-y-1.5">
                                <div className="grid grid-cols-[80px_1fr_100px_1fr] gap-1.5 text-xs font-semibold text-gray-400 uppercase px-0.5"><span>Session</span><span>Activity</span><span>Status</span><span>Experience</span></div>
                                {domainRows.map(({ sIdx, rIdx, row }) => (
                                  <div key={`${sIdx}-${rIdx}`} className="grid grid-cols-[80px_1fr_100px_1fr] gap-1.5">
                                    <span className="px-5 py-1.5 text-xs text-gray-500 flex items-center gap-1"><span>{SESSION_ICONS[planner.plans[sIdx].session]}</span></span>
                                    ...
                                    <span className="px-1 py-1.5 border border-gray-200 rounded text-xs bg-gray-50 text-gray-600 text-center">{row.completed || '—'}</span>
                                    <span className="px-2 py-1.5 border border-gray-200 rounded text-xs bg-gray-50 text-gray-600">{row.experience || '—'}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                          <div className="rounded-lg border border-gray-200 p-4"><div className="flex items-center justify-between mb-2"><label className="text-sm font-semibold text-gray-600">✨ Blessings earned for the day!</label><StarRating size="sm" value={planner.blessingsRating} /></div><p className="text-sm text-gray-700 whitespace-pre-wrap">{planner.blessings || <span className="text-gray-300 italic">—</span>}</p></div>
                          <div className="rounded-lg border border-gray-200 p-4"><div className="flex items-center justify-between mb-2"><label className="text-sm font-semibold text-gray-600">😊 Happy moment of the day</label><StarRating size="sm" value={planner.happyMomentRating} /></div><p className="text-sm text-gray-700 whitespace-pre-wrap">{planner.happyMoment || <span className="text-gray-300 italic">—</span>}</p></div>
                        </div>
                      </div>
                    )}
                    {activeTab === 3 && (
                          ...
                          <div className="rounded-lg border border-gray-200 p-4 space-y-2.5">
                            ...
                            {planner.nightLog.newWords.map((word, idx) => (
                              <div key={idx} className="flex items-center gap-2">
                                ...
                                <span className="flex-1 px-2.5 py-1.5 border border-gray-200 rounded text-sm bg-gray-50 text-gray-600">{word || <span className="text-gray-300 italic">—</span>}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="rounded-lg border border-gray-200 p-4 space-y-3">
                            ...
                            {[{ key: 'sleepRating', label: '😴 Sleep', note: 'How you slept last night' }, { key: 'exerciseRating', label: '🏃 Exercise' }, { key: 'dietRating', label: '🥗 Diet' }, { key: 'studyTimeRating', label: '📖 Study Time' }, { key: 'productivityRating', label: '⚡ Productivity' }].map(({ key, label, note }) => (
                              <div key={key} className="flex items-center justify-between">
                                <div><span className="text-sm text-gray-700">{label}</span>{note && <p className="text-[10px] text-gray-400">{note}</p>}</div>
                                <StarRating value={(planner.selfCare as unknown as Record<string, number>)[key]} />
                              </div>
                            ))}
                          </div>
                          <div className="space-y-3">
                            <div className="rounded-lg border border-gray-200 p-3"><div className="flex items-center justify-between mb-1.5"><label className="text-sm font-semibold text-gray-600">🪻 Self-Reflection / Self-Advice</label><StarRating size="sm" value={planner.selfCare.selfReflectionRating} /></div><p className="text-sm text-gray-700 whitespace-pre-wrap">{planner.selfCare.selfReflection || <span className="text-gray-300 italic">—</span>}</p></div>
                            <div className="rounded-lg border border-gray-200 p-3"><div className="flex items-center justify-between mb-1.5"><label className="text-sm font-semibold text-gray-600">🛠 Skills I used today</label><StarRating size="sm" value={planner.selfCare.skillsUsedRating} /></div><p className="text-sm text-gray-700 whitespace-pre-wrap">{planner.selfCare.skillsUsed || <span className="text-gray-300 italic">—</span>}</p></div>
                            <div className="rounded-lg border border-gray-200 p-3"><div className="flex items-center justify-between mb-1.5"><label className="text-sm font-semibold text-gray-600">🏅 My Achievements</label><StarRating size="sm" value={planner.selfCare.achievementsRating} /></div><p className="text-sm text-gray-700 whitespace-pre-wrap">{planner.selfCare.achievements || <span className="text-gray-300 italic">—</span>}</p></div>
                          </div>
                        </div>
                      </div>
                    )}
                      ...
                      return (
                        <div className="mt-5 pt-4 border-t-2 border-blue-100">
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <div><h3 className="text-sm font-bold text-gray-800">Weekly Feedback</h3><p className="text-[10px] text-gray-400">{format(new Date(monday + 'T12:00:00'), 'MMM d')} – {format(new Date(sunday + 'T12:00:00'), 'MMM d, yyyy')}</p></div>
                          </div>
                          {weeklyFeedbacks.length > 0 ? (
                            <div className="space-y-2">
                              {weeklyFeedbacks.map((fb) => (
                                <div key={fb._id} className="rounded-lg border border-blue-100 bg-blue-50/50 p-3">
                                  <div className="flex items-center justify-between mb-1"><span className="text-xs font-semibold text-blue-700">{fb.givenByName}</span><span className="text-[10px] text-gray-400">{format(new Date(fb.createdAt), 'MMM d, yyyy h:mm a')}</span></div>
                                  <p className="text-sm text-gray-700 whitespace-pre-wrap">{fb.feedback}</p>
                                </div>
                              ))}
                            </div>
              ...
              ) : (<div className="flex items-center justify-center py-16"><div className="w-7 h-7 border-3 border-blue-200 border-t-blue-600 rounded-full animate-spin" /></div>)
            ) : (
...
export default function ReferrerStudentActivityPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
