    ...
    try {
      ...
      if (res.data.data) {
        ...
        setPlanner({
          ...
          goalHabitFocus: d.goalHabitFocus || '', goalPsychological: d.goalPsychological || '',
          goalPhysical: d.goalPhysical || '', goalReadingBooks: d.goalReadingBooks || '',
          wakeupTime: d.wakeupTime || '', lunchTime: d.lunchTime || '', dinnerTime: d.dinnerTime || '', sleepTime: d.sleepTime || '',
          plans: d.plans?.length
            ? d.plans.map((p: SessionPlan) => ({
                session: p.session,
                rows: p.rows?.length ? p.rows.map((r: PlanRow) => ({ ...defaultRow(), ...r })) : [defaultRow(), defaultRow()],
              }))
            : JSON.parse(JSON.stringify(emptyPlanner.plans)),
          ...
          nightLog: d.nightLog ? { ...emptyPlanner.nightLog, ...d.nightLog, newWords: d.nightLog.newWords?.length === 5 ? d.nightLog.newWords : ['', '', '', '', ''] } : JSON.parse(JSON.stringify(emptyPlanner.nightLog)),
          selfCare: d.selfCare ? { ...emptyPlanner.selfCare, ...d.selfCare } : JSON.parse(JSON.stringify(emptyPlanner.selfCare)),
        });
      } else {
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50/30 p-4 md:p-6 animate-fadeIn">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push(`/advisory/students/${studentId}/registration/${registrationId}`)}
...
export default function AdvisoryStudentActivityPage() {
  return (
    <Suspense fallback={
      <AdvisoryLayout>
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
