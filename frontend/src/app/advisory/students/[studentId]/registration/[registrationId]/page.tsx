...
import { authAPI, advisoryAPI, programAPI, teamMeetAPI, opsScheduleAPI, activityAPI } from '@/lib/api';
...
export default function AdvisoryStudentFormViewPage() {
  ...
  const handleTeamMeetSave = async () => {
    ...
    setSelectedTeamMeet(null);
    await fetchTeamMeetsForStudent();
  };
  ...
  const fetchAllData = async () => {
    try {
      const studentResponse = await advisoryAPI.getStudentDetail(studentId);
      const studentData = studentResponse.data.data.student;
      setStudentInfo(studentData);

      const registrationResponse = await advisoryAPI.getStudentFormAnswers(studentId, registrationId);
        ...
        <div className="p-8">
          ...
          {studentInfo && serviceInfo && (
            <StudentFormHeader
              ...
              adminId={studentInfo.adminId?._id || studentInfo.advisoryId?._id}
            />
          )}
          ...
          {isEducationPlanning && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-6 overflow-hidden">
              <div className="flex border-b border-gray-200">
                ...
                <button onClick={() => router.push(`/advisory/students/${studentId}/registration/${registrationId}/activity`)}
          ...
          {isEducationPlanning && activeView === 'portfolio' && (
            <div className="mb-6">
              ...
              {portfolios.length > 0 && (
                <div className="mt-4 border border-indigo-200 rounded-xl p-5 bg-indigo-50/50">
