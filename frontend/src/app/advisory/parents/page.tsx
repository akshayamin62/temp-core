...
export default function AdvisoryParentsPage() {
  ...
  const fetchParents = async () => {
    try {
      const response = await advisoryAPI.getParents();
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              {p.relationship && <div className="text-sm text-gray-500">{p.relationship}</div>}
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-1">
                            {p.studentIds?.map((s: any) => (
                              <span key={s._id} className="px-2 py-0.5 text-xs font-medium bg-blue-100 text-blue-800 rounded-full">
                                {getFullName(s.userId)}
                              </span>
                            ))}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-3 py-1 text-xs font-medium rounded-full ${p.userId?.isActive !== false ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                            {p.userId?.isActive !== false ? 'Active' : 'Inactive'}
                          </span>
                        </td>
                      </tr>
                    ))
