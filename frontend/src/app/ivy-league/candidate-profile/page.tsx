...
function ReadOnlyStars({ score }: { score: number }) {
  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg key={star} className={`w-6 h-6 ${star <= score ? 'text-amber-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 20 20">
...
function CandidateProfileContent() {
  ...
  useEffect(() => {
    ...
    if (userIdParam) {
      ...
      authAPI.getProfile().then((res) => {
        fetchData(res.data.data.user._id);
  ...
  const fetchData = async (uid: string) => {
    try {
      ...
      // Try to find student info (best-effort — not all roles have access)
      try {
        const studentsRes = await axios.get(`${API_URL}/super-admin/ivy-league/students`, { headers });
        let found: StudentInfo | null = studentsRes.data.students?.find((s: any) => s.userId?.toString() === uid) ?? null;
        if (!found) {
          const candidatesRes = await axios.get(`${API_URL}/super-admin/ivy-league/candidates`, { headers });
          found = candidatesRes.data.candidates?.find((c: any) => c.userId?.toString() === uid) ?? null;
        }
        if (found) setStudent(found);
      } catch {
        // Roles without access to admin list — skip silently
      }

      // Directly fetch test result — primary data source
      try {
        const testRes = await axios.get(`${API_URL}/super-admin/ivy-league/test-result/${uid}`, { headers });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm font-medium">Loading candidate profile...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        {/* Back */}
        <button onClick={() => router.back()} className="mb-6 inline-flex items-center gap-2 text-sm text-gray-600 hover:text-brand-600 transition-colors">
        ...
        {/* Title */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">{student ? getFullName(student) : 'Ivy League Candidate Profile'}</h1>
          {student && <p className="text-gray-600 mt-1">{student.schoolName} • Grade {student.currentGrade} • {student.curriculum}</p>}
        </div>

        {/* Not registered */}
        {!hasData && !student && !testResult && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p className="text-lg font-medium text-gray-500">No Ivy League data found</p>
            <p className="text-sm text-gray-400 mt-2">No Ivy League registration or test result found for this student.</p>
          </div>
        )}

        {/* Show content if we have any data */}
        {(student || testResult) && (
          <>
            {/* Student info card */}
            {student && (
            ...
            <div className="flex gap-2 mb-6 flex-wrap">
              {([
                { key: 'test', label: '📝 Test Score', active: 'bg-blue-600 text-white', inactive: 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' },
                { key: 'student-interview', label: '🎓 Student Interview', active: 'bg-green-600 text-white', inactive: 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' },
                { key: 'parent-interview', label: '👨‍👩‍👧 Parent Interview', active: 'bg-purple-600 text-white', inactive: 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' },
              ] as const).map((tab) => (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`px-5 py-2.5 rounded-lg font-semibold text-sm transition-colors ${activeTab === tab.key ? tab.active : tab.inactive}`}>
                  {tab.label}
                </button>
              ))}
            </div>
                ...
                {!testResult ? (
                  ...
                  <>
                    ...
                    {(() => {
                      ...
                      const radarData = testResult.sections.map((sec) => ({ subject: sec.sectionName.split(' ').slice(0, 2).join(' '), score: sec.questionCount > 0 ? Math.round((sec.score / (sec.questionCount * 2)) * 100) : 0, fullMark: 100 }));
                      const barData = testResult.sections.map((sec, i) => ({ name: sec.sectionName.split(' ').slice(0, 2).join(' '), 'Your Score': sec.score, 'Max Marks': sec.questionCount * 2, fill: CHART_COLORS[i % CHART_COLORS.length] }));
                      const donutData = [{ name: 'Correct', value: totalCorrect, color: '#22c55e' }, { name: 'Incorrect', value: totalIncorrect, color: '#ef4444' }, { name: 'Skipped', value: totalSkipped, color: '#9ca3af' }];
                      const accuracyData = testResult.sections.map((sec, i) => { const attempted = sec.correct + sec.incorrect; return { name: sec.sectionName.split(' ').slice(0, 2).join(' '), accuracy: attempted > 0 ? Math.round((sec.correct / attempted) * 100) : 0, fill: CHART_COLORS[i % CHART_COLORS.length] }; });
                      return (
                        <div className="mb-6">
                      ...
                      {testResult.sections.map((sec, idx) => (
                        <div key={idx} className="flex-1 min-w-[200px] bg-white rounded-xl shadow-sm border border-gray-200 p-5 cursor-pointer hover:shadow-md transition-all" style={{ borderLeftWidth: 4, borderLeftColor: SECTION_COLORS[idx] || '#6b7280' }} onClick={() => setActiveSectionIdx(idx)}>
                          <div className="flex items-center justify-between mb-3">
                            <p className="text-base font-bold text-gray-900">{SECTION_ICONS[idx]} {sec.sectionName}</p>
                            <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${sec.status === 'submitted' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{sec.status === 'submitted' ? 'Submitted' : sec.status}</span>
                          </div>
