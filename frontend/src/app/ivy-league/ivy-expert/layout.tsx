...
function IvyExpertSidebar() {
    ...
    // Get IDs from URL params (passed by auth/role routing)
    ...
    const navItems = [
        {
            name: 'Dashboard', href: `/ivy-league/ivy-expert${queryString}`, icon: (
        ...
        {
            name: 'Documents', href: `/ivy-league/ivy-expert/documents${queryString}`, icon: (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {
            name: 'Academic Excellence', href: `/ivy-league/ivy-expert/pointer1${queryString}`, icon: (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {
            name: 'Spike in one area', href: `/ivy-league/ivy-expert/activities${queryString}&pointerNo=2`, icon: (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {
            name: 'Leadership & Initiative', href: `/ivy-league/ivy-expert/activities${queryString}&pointerNo=3`, icon: (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {
            name: 'Global or Social Impact', href: `/ivy-league/ivy-expert/activities${queryString}&pointerNo=4`, icon: (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {
            name: 'Authentic & Reflective Storytelling', href: `/ivy-league/ivy-expert/pointer5${queryString}`, icon: (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {
            name: 'Engagement with Learning & Intellectual Curiosity', href: `/ivy-league/ivy-expert/pointer6${queryString}`, icon: (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <aside className={`bg-white border-r border-gray-100 flex flex-col h-screen sticky top-0 shadow-sm z-20 transition-all duration-300 ${isConversationOpen ? 'w-20' : 'w-72'}`}>
            <div className={`p-8 border-b border-gray-50 ${isConversationOpen ? 'px-4' : ''}`}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mt-1">Ivy Expert Portal</p>
                    </div>
                    )}
                </div>
            </div>
            ...
            <div className="p-6">
                <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100">
                    ...
                    <p className="text-xs font-bold text-gray-900 leading-relaxed uppercase tracking-tight">Ivy Expert</p>
