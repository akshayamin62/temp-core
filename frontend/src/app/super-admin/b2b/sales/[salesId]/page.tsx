...
export default function B2BSalesDetailPage() {
  ...
  const fetchDashboard = async () => {
    try {
      const [dashRes, followUpsRes, summaryRes] = await Promise.all([
        b2bAPI.getSalesDashboard(salesId),
        b2bAPI.getSalesFollowUps(salesId),
        b2bAPI.getSalesFollowUpSummary(salesId),
      ]);
      ...
      console.error('Error fetching B2B Sales dashboard:', error);
      toast.error(error.response?.data?.message || 'Failed to fetch dashboard');
      router.push('/super-admin/b2b/sales');
    }
  };
