...
export default function SuperAdminAdvisorDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const response = await superAdminAPI.getAdvisorDashboardStats(advisorId);
      setStats(response.data.data);
    } catch (error: any) {
      console.error('Fetch dashboard error:', error);
      toast.error(error.response?.data?.message || 'Failed to fetch advisor dashboard');
      router.push('/super-admin/roles/advisor');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchTeamMeets = useCallback(async () => {
    try {
      const response = await superAdminAPI.getAdvisorTeamMeets(advisorId);
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                  <ActionCard
                    ...
                    onClick={() => router.push(`/super-admin/roles/advisor/${advisorId}/leads`)}
                    color="blue"
                  />
                  <ActionCard
                    title="View Students"
                    description="Check all students under this advisor"
                    icon={
                      <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 14l9-5-9-5-9 5 9 5z" />
                    ...
                    onClick={() => router.push(`/super-admin/roles/advisor/${advisorId}/students`)}
                    color="blue"
                  />
                  <ActionCard
                    title="Service Pricing"
                    description="View this advisor's service pricing"
                    icon={
                      <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    onClick={() => router.push(`/super-admin/roles/advisor/${advisorId}/service-pricing`)}
                    color="purple"
                  />
                  {stats?.advisor?.b2bLeadId && (
                    <ActionCard
                      ...
                      onClick={() => router.push(`/super-admin/b2b/leads/${stats.advisor.b2bLeadId}/profile`)}
                      color="green"
                    />
                  )}
                </div>
