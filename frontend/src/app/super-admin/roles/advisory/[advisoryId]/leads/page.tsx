...
export default function SuperAdminAdvisoryLeadsPage() {
  ...
  const fetchLeads = async () => {
    try {
      ...
      const response = await superAdminAPI.getAdvisoryLeads(advisoryId, params);
    ...
    <>
      ...
      <SuperAdminLayout user={currentUser}>
        <div className="p-8">
          {/* Back Button */}
          <button
            onClick={() => router.push(`/super-admin/roles/advisory/${advisoryId}`)}
          ...
          {/* Header */}
          <div className="mb-8">
            <h2 className="text-3xl font-bold text-gray-900">Advisory&apos;s Leads</h2>
            <p className="text-gray-600 mt-1">View leads under this advisory (read-only)</p>
          </div>
