...
export default function SuperAdminAdvisoryDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const response = await superAdminAPI.getAdvisoryDashboardStats(advisoryId);
  ...
  const fetchTeamMeets = useCallback(async () => {
    try {
      const response = await superAdminAPI.getAdvisoryTeamMeets(advisoryId);
      ...
      <SuperAdminLayout user={currentUser}>
        <div className="p-8">
          ...
          <div className="flex items-center gap-4 mb-6">
            <button
              onClick={() => router.push('/super-admin/roles/advisory')}
            ...
            <div className="flex items-center gap-3">
              {stats?.advisory?.companyLogo ? (
                <img
                  src={`${API_URL}/${stats.advisory.companyLogo.replace(/^\//, '')}`}
                  alt={stats.advisory.companyName || 'Company Logo'}
                ...
                <div className="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center">
                  <span className="text-lg font-bold text-blue-600">
                    {stats?.advisory?.companyName?.charAt(0) || getInitials(stats?.advisory) || 'A'}
                  </span>
                </div>
              )}
              <div>
            ...
            <div className="flex flex-wrap gap-6 text-sm">
              <div>
                ...
                <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${
                  stats?.advisory?.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                }`}>
                  {stats?.advisory?.isActive ? 'Active' : 'Inactive'}
                </span>
              </div>
              <div>
                <span className="text-gray-500">Verified: </span>
                <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${
                  stats?.advisory?.isVerified ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800'
                }`}>
                  {stats?.advisory?.isVerified ? 'Yes' : 'No'}
                </span>
              </div>
              {stats?.advisory?.mobileNumber && (
                <div>
                  <span className="text-gray-500">Phone: </span>
                  <span className="text-gray-900">{stats.advisory.mobileNumber}</span>
                </div>
              )}
              {stats?.advisory?.address && (
                <div>
                  <span className="text-gray-500">Address: </span>
                  <span className="text-gray-900">{stats.advisory.address}</span>
                </div>
              )}
            ...
            <>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <ActionCard
                    title="View Leads"
                    description="Check all leads from this advisory's enquiry form"
                    icon={
                    ...
                    onClick={() => router.push(`/super-admin/roles/advisory/${advisoryId}/leads`)}
                    color="blue"
                  />
                  <ActionCard
                    title="View Students"
                    description="Check all students under this advisory"
                    icon={
                      <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 14l9-5-9-5-9 5 9 5z" />
                    ...
                    onClick={() => router.push(`/super-admin/roles/advisory/${advisoryId}/students`)}
                    color="blue"
                  />
                  <ActionCard
                    title="Service Pricing"
                    description="View this advisory's service plan pricing"
                    icon={
                      <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    onClick={() => router.push(`/super-admin/roles/advisory/${advisoryId}/service-pricing`)}
                    color="purple"
                  />
                </div>
              </div>
