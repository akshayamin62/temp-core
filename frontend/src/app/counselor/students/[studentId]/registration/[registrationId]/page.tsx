        ...
        <div className="p-8">
              ...
              <div className="px-4 py-3 flex flex-wrap items-center gap-2">
                {navButtons.map((btn) => (
                  <button key={btn.key} onClick={() => setActiveView(btn.key)}
                    className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${activeView === btn.key ? 'bg-teal-600 text-white shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 border border-gray-200'}`}>
                    <span>{btn.icon}</span> {btn.label}
                  </button>
                ))}
                {formStructure.length > 0 && (
                  <button onClick={() => setActiveView('form')}
                    className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${activeView === 'form' ? 'bg-teal-600 text-white shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 border border-gray-200'}`}>
                    <span>📋</span> Form
                  </button>
                )}
                <button onClick={() => router.push(`/counselor/students/${studentId}/registration/${registrationId}/activity`)}
                  className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-semibold bg-teal-50 text-teal-700 hover:bg-teal-100 border border-teal-200 transition-all duration-200 ml-auto">
                  ...
                  <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={handleBrainographyView} className="px-3 py-1.5 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors text-xs font-medium">View</button>
                        <button onClick={handleBrainographyDownload} className="px-3 py-1.5 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors text-xs font-medium">Download</button>
                      </div>
                    </div>
                  </div>
                ) : (
          ...
          {isEducationPlanning && activeView === 'portfolio' && (
            <div className="mb-6">
              ...
              {portfolios.length > 0 && (
                <div className="mt-4 border border-teal-200 rounded-xl p-5 bg-teal-50/50">
                  ...
                  {currentPart.key === 'APPLICATION' && (currentSection.title === 'Apply to Program' || currentSection.title === 'Applied Program') ? (
                    <div className="bg-white rounded-lg border border-gray-200 p-6">
                      <div className="bg-teal-600 px-6 py-4 -mx-6 -mt-6 mb-6 border-b border-teal-700">
                        ...
                        {currentSection.description && <p className="text-teal-100 text-sm mt-1">{currentSection.description}</p>}
                      </div>
