...
export default function CounselorLeadsPage() {
  ...
  // Conversion request
  const [requestingConversion, setRequestingConversion] = useState<string | null>(null);
  ...
  const fetchEnquiryFormUrl = async () => {
    try {
      const response = await leadAPI.getCounselorEnquiryFormUrl();
  ...
  // Request conversion
  const handleRequestConversion = async (lead: Lead) => {
    try {
      setRequestingConversion(lead._id);
      await leadConversionAPI.requestConversion(lead._id);
      toast.success(`Conversion request sent for ${lead.name}`);
      fetchLeads();
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Failed to request conversion');
    } finally {
      setRequestingConversion(null);
    }
  };
                      ...
                      <tr key={lead._id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-4 text-center">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            {lead.stage !== LEAD_STAGE.CONVERTED && lead.stage !== LEAD_STAGE.CLOSED && !lead.conversionStatus && (
                              <button
                              ...
                              >
                                {requestingConversion === lead._id ? 'Requesting...' : 'Request Conversion'}
                              </button>
                            )}
                          </div>
                        </td>
