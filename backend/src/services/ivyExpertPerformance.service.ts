...
export const getIvyExpertPerformance = async (): Promise<IvyExpertMetrics[]> => {
    // 1. Get all IvyExpert records with their User data
    const ivyExpertRecords = await IvyExpert.find({}).populate('userId', 'firstName lastName email');

    const metrics: IvyExpertMetrics[] = [];

    for (const expert of ivyExpertRecords) {
        const user = expert.userId as any; // populated User doc
        if (!user) continue;

        // 2. Get students handled (registrations where this IvyExpert is the active ivy expert)
        const services = await StudentServiceRegistration.find({ activeIvyExpertId: expert._id });
        ...
        const activityEvaluations = await IvyExpertEvaluation.find({
