...
exports.getCounselorTeamMeetsForSuperAdmin = exports.getCounselorFollowUpSummaryForSuperAdmin = exports.getCounselorFollowUpsForSuperAdmin = exports.getCounselorDetailForSuperAdmin = exports.getAllLeadsForSuperAdmin = exports.getAdminTeamMeetsForSuperAdmin = exports.getAdminStudentsForSuperAdmin = exports.getAdminLeadsForSuperAdmin = exports.getAdminCounselorsForSuperAdmin = exports.getAdminDashboardStats = exports.createUserByRole = exports.getAdminDetails = exports.getAdmins = exports.createAdmin = exports.getAllEduplanCoaches = exports.getAllIvyExperts = exports.getAllOps = exports.createOps = exports.getPendingApprovals = exports.deleteUser = exports.toggleUserStatus = exports.rejectUser = exports.approveUser = exports.getUserStats = exports.getAllUsers = void 0;
const User_1 = __importDefault(require("../models/User"));
const roles_1 = require("../types/roles");
const Ops_1 = __importDefault(require("../models/Ops"));
const IvyExpert_1 = __importDefault(require("../models/IvyExpert"));
const EduplanCoach_1 = __importDefault(require("../models/EduplanCoach"));
...
const TeamMeet_1 = __importDefault(require("../models/TeamMeet"));
    ...
    try {
        ...
        if (!role || (role && String(role).toUpperCase() === 'ADMIN')) {
            enrichedUsers = await Promise.all(users.map(async (user) => {
                ...
                if (userObj.role === roles_1.USER_ROLE.ADMIN) {
                    const adminProfile = await Admin_1.default.findOne({ userId: user._id }).select('companyName companyLogo');
                    if (adminProfile) {
        ...
        if (role && String(role).toUpperCase() === 'COUNSELOR') {
            enrichedUsers = await Promise.all(users.map(async (user) => {
                const userObj = user.toObject();
                if (userObj.role === roles_1.USER_ROLE.COUNSELOR) {
                    const counselorProfile = await Counselor_1.default.findOne({ userId: user._id });
                    if (counselorProfile) {
                        const adminProfile = await Admin_1.default.findOne({ userId: counselorProfile.adminId }).select('companyName');
                        if (adminProfile) {
                            userObj.companyName = adminProfile.companyName;
                        }
                    }
            ...
            if (search) {
                ...
                enrichedUsers = enrichedUsers.filter((user) => (user.firstName || '').toLowerCase().includes(searchLower) ||
...
const getUserStats = async (_req, res) => {
    try {
        ...
        const pendingApproval = await User_1.default.countDocuments({
            isVerified: false,
            role: { $nin: [roles_1.USER_ROLE.SUPER_ADMIN, roles_1.USER_ROLE.STUDENT] }, // Unverified users except admins and students
        });
...
const createOps = async (req, res) => {
    try {
        ...
        // Create ops user
        const opsUser = await User_1.default.create({
            firstName: firstName.trim(),
            middleName: middleName?.trim() || undefined,
            lastName: lastName.trim(),
            email: email.toLowerCase().trim(),
            role: roles_1.USER_ROLE.OPS,
            isVerified: true, // Auto-verify ops created by admin
            isActive: true,
        });
        // Create ops record
        const OpsModel = (await Promise.resolve().then(() => __importStar(require("../models/Ops")))).default;
 ...
 * Get all ops
 */
const getAllOps = async (_req, res) => {
    try {
        const ops = await Ops_1.default.find()
 ...
 * Get all Ivy Experts
 */
const getAllIvyExperts = async (_req, res) => {
    try {
        const ivyExperts = await IvyExpert_1.default.find()
            .populate('userId', 'firstName middleName lastName email isActive')
            .sort({ createdAt: -1 });
        return res.status(200).json({
            success: true,
...
exports.getAllIvyExperts = getAllIvyExperts;
/**
 * Get all Eduplan Coaches
 */
const getAllEduplanCoaches = async (_req, res) => {
    try {
        const eduplanCoaches = await EduplanCoach_1.default.find()
            .populate('userId', 'firstName middleName lastName email isActive')
            .sort({ createdAt: -1 });
        return res.status(200).json({
            success: true,
            ...
            data: {
                eduplanCoaches: eduplanCoaches.map((c) => ({
                    _id: c._id,
                    userId: c.userId,
                    email: c.email,
                    mobileNumber: c.mobileNumber,
...
exports.getAllEduplanCoaches = getAllEduplanCoaches;
/**
 * Create a new Admin
 */
...
const getAdmins = async (_req, res) => {
    try {
        const admins = await User_1.default.find({ role: roles_1.USER_ROLE.ADMIN, isActive: true })
...
const getAdminDetails = async (req, res) => {
    try {
        ...
        const user = await User_1.default.findOne({ _id: adminId, role: roles_1.USER_ROLE.ADMIN });
        if (!user) {
            return res.status(404).json({
                success: false,
        ...
        const adminProfile = await Admin_1.default.findOne({ userId: adminId });
        if (!adminProfile) {
            return res.status(404).json({
                success: false,
...
const createUserByRole = async (req, res) => {
    try {
        ...
        const companyLogo = req.file ? `/uploads/admin/${req.file.filename}` : undefined;
        // Validation
        ...
        if (role === roles_1.USER_ROLE.ADMIN && !companyName) {
            return res.status(400).json({
                success: false,
        ...
        const allowedRoles = [
            roles_1.USER_ROLE.ADMIN,
            roles_1.USER_ROLE.OPS,
            roles_1.USER_ROLE.EDUPLAN_COACH,
            roles_1.USER_ROLE.IVY_EXPERT,
            roles_1.USER_ROLE.COUNSELOR,
        ];
        ...
        if (role === roles_1.USER_ROLE.COUNSELOR && !adminId) {
            return res.status(400).json({
                success: false,
        ...
        if (role === roles_1.USER_ROLE.COUNSELOR && adminId) {
            const adminUser = await User_1.default.findOne({ _id: adminId, role: roles_1.USER_ROLE.ADMIN });
            if (!adminUser) {
                return res.status(400).json({
                    success: false,
        ...
        if (role === roles_1.USER_ROLE.ADMIN) {
            // Generate slug from company name, falling back to custom slug or full name
            let baseSlug;
            if (customSlug) {
                baseSlug = (0, leadController_1.generateSlug)(customSlug);
            }
            else if (companyName) {
                baseSlug = (0, leadController_1.generateSlug)(companyName);
            }
            else {
                const fullName = [firstName, middleName, lastName].filter(Boolean).join(' ');
                baseSlug = (0, leadController_1.generateSlug)(fullName);
            }
            enquiryFormSlug = await (0, leadController_1.getUniqueSlug)(baseSlug);
            const newAdmin = new Admin_1.default({
                userId: newUser._id,
                email: email.toLowerCase().trim(),
                mobileNumber: phoneNumber?.trim() || undefined,
        ...
        if (role === roles_1.USER_ROLE.OPS) {
            const newOps = new Ops_1.default({
                userId: newUser._id,
                email: email.toLowerCase().trim(),
                mobileNumber: phoneNumber?.trim() || undefined,
            });
        ...
        if (role === roles_1.USER_ROLE.IVY_EXPERT) {
            const newIvyExpert = new IvyExpert_1.default({
                userId: newUser._id,
                email: email.toLowerCase().trim(),
                mobileNumber: phoneNumber?.trim() || undefined,
            });
        ...
        if (role === roles_1.USER_ROLE.EDUPLAN_COACH) {
            const newEduplanCoach = new EduplanCoach_1.default({
                userId: newUser._id,
                email: email.toLowerCase().trim(),
                mobileNumber: phoneNumber?.trim() || undefined,
            });
        ...
        if (role === roles_1.USER_ROLE.COUNSELOR) {
...
const getAdminDashboardStats = async (req, res) => {
    try {
        ...
        // Get counselor count
        const totalCounselors = await Counselor_1.default.countDocuments({ adminId: adminId });
        // Get lead stats
        const allLeads = await Lead_1.default.find({ adminId: adminId });
        const totalLeads = allLeads.length;
        const newLeads = allLeads.filter((l) => l.stage === Lead_1.LEAD_STAGE.NEW).length;
        // Get student count
        const totalStudents = await Student_1.default.countDocuments({ adminId: adminProfile._id });
        // Enquiry form URL
 ...
 */
const getAdminCounselorsForSuperAdmin = async (req, res) => {
    try {
        const { adminId } = req.params;
        const adminUser = await User_1.default.findOne({ _id: adminId, role: roles_1.USER_ROLE.ADMIN });
        if (!adminUser) {
        ...
        const counselors = await Counselor_1.default.find({ adminId: adminId })
...
exports.getAdminCounselorsForSuperAdmin = getAdminCounselorsForSuperAdmin;
/**
 * Get leads under a specific admin (for super admin)
 */
const getAdminLeadsForSuperAdmin = async (req, res) => {
    try {
        const { adminId } = req.params;
...
exports.getAdminLeadsForSuperAdmin = getAdminLeadsForSuperAdmin;
/**
 * Get students under a specific admin (for super admin)
 */
const getAdminStudentsForSuperAdmin = async (req, res) => {
    try {
        const { adminId } = req.params;
        const adminUser = await User_1.default.findOne({ _id: adminId, role: roles_1.USER_ROLE.ADMIN });
        if (!adminUser) {
        ...
        const students = await Student_1.default.find({ adminId: adminProfile._id })
        ...
        // Calculate stats
        const activeStudents = studentsWithStats.filter((s) => s.user?.isActive).length;
        return res.status(200).json({
            success: true,
            data: {
...
exports.getAdminStudentsForSuperAdmin = getAdminStudentsForSuperAdmin;
/**
 * Get team meets for a specific admin (for super admin - read only)
 */
const getAdminTeamMeetsForSuperAdmin = async (req, res) => {
    try {
        const { adminId } = req.params;
...
exports.getAdminTeamMeetsForSuperAdmin = getAdminTeamMeetsForSuperAdmin;
// ============= ALL LEADS FOR SUPER ADMIN =============
/**
 * Get all leads across all admins (for super admin)
 */
const getAllLeadsForSuperAdmin = async (req, res) => {
    try {
        ...
        const adminUserIds = [...new Set(leads.map((l) => l.adminId?._id?.toString()).filter(Boolean))];
        const admins = await Admin_1.default.find({ userId: { $in: adminUserIds } }).select("userId companyName");
        const adminCompanyMap = {};
        admins.forEach((a) => {
...
exports.getAllLeadsForSuperAdmin = getAllLeadsForSuperAdmin;
// ============= COUNSELOR DASHBOARD FOR SUPER ADMIN =============
/**
 * Get counselor detail with dashboard data (for super admin)
 */
const getCounselorDetailForSuperAdmin = async (req, res) => {
    try {
        const { counselorId } = req.params;
        // Find counselor by _id first, then fallback to userId
        let counselor = await Counselor_1.default.findById(counselorId)
            .populate('userId', 'firstName middleName lastName name email isActive isVerified');
        if (!counselor) {
            counselor = await Counselor_1.default.findOne({ userId: counselorId })
                .populate('userId', 'firstName middleName lastName name email isActive isVerified');
        }
        if (!counselor) {
...
exports.getCounselorDetailForSuperAdmin = getCounselorDetailForSuperAdmin;
/**
 * Get counselor's follow-ups (for super admin)
 */
const getCounselorFollowUpsForSuperAdmin = async (req, res) => {
    try {
        const { counselorId } = req.params;
        ...
        const followUps = await FollowUp_1.default.find({ counselorId: counselor._id })
...
exports.getCounselorFollowUpsForSuperAdmin = getCounselorFollowUpsForSuperAdmin;
/**
 * Get counselor's follow-up summary (for super admin)
 */
const getCounselorFollowUpSummaryForSuperAdmin = async (req, res) => {
    try {
        const { counselorId } = req.params;
        let counselor = await Counselor_1.default.findById(counselorId);
        if (!counselor) {
...
exports.getCounselorFollowUpSummaryForSuperAdmin = getCounselorFollowUpSummaryForSuperAdmin;
/**
 * Get counselor's team meets (for super admin)
 */
const getCounselorTeamMeetsForSuperAdmin = async (req, res) => {
    try {
        const { counselorId } = req.params;
        let counselor = await Counselor_1.default.findById(counselorId)
            .populate('userId', '_id');
        ...
        const counselorUserId = counselor.userId?._id;
        const teamMeets = await TeamMeet_1.default.find({
...
exports.getCounselorTeamMeetsForSuperAdmin = getCounselorTeamMeetsForSuperAdmin;
