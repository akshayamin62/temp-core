...
exports.switchActiveOps = exports.assignOps = exports.getStudentsWithRegistrations = exports.updateStudentFormAnswers = exports.getStudentFormAnswers = exports.getStudentDetails = exports.getAllStudents = void 0;
...
const getAllStudents = async (req, res) => {
    try {
        ...
        if (userRole === roles_1.USER_ROLE.SUPER_ADMIN) {
            // Get all registrations with their service info
            const allRegistrations = await StudentServiceRegistration_1.default.find({})
                .populate('serviceId', 'name slug');
            for (const registration of allRegistrations) {
                const service = registration.serviceId;
                if (!service)
                    continue;
 ...
 */
const getStudentFormAnswers = async (req, res) => {
    try {
        ...
        if (user?.role === roles_1.USER_ROLE.OPS) {
            const ops = await Ops_1.default.findOne({ userId }).lean().exec();
            if (!ops) {
                return res.status(404).json({
                    success: false,
                    message: 'OPS record not found',
            ...
            const fullRegistration = await StudentServiceRegistration_1.default.findById(registrationId)
        ...
        const student = await Student_1.default.findById(studentId).lean().exec();
        return res.status(200).json({
            success: true,
...
exports.getStudentFormAnswers = getStudentFormAnswers;
/**
 * Update student form answers (admin/OPS can edit)
 */
const updateStudentFormAnswers = async (req, res) => {
    try {
...
exports.updateStudentFormAnswers = updateStudentFormAnswers;
/**
 * Get all students with service registrations (for dropdown/selection)
 */
const getStudentsWithRegistrations = async (req, res) => {
    try {
        const { serviceId } = req.query;
        let query = {};
        if (serviceId) {
            // Get students registered for a specific service
            const registrations = await StudentServiceRegistration_1.default.find({ serviceId }).select('studentId');
...
exports.getStudentsWithRegistrations = getStudentsWithRegistrations;
/**
...
const assignOps = async (req, res) => {
    try {
        ...
        if (serviceName === 'Study Abroad') {
            ...
            if (primaryOpsId) {
                const primaryOps = await Ops_1.default.findById(primaryOpsId);
                if (!primaryOps) {
                    return res.status(404).json({
                        success: false,
            ...
            if (secondaryOpsId) {
                const secondaryOps = await Ops_1.default.findById(secondaryOpsId);
                if (!secondaryOps) {
                    return res.status(404).json({
                        success: false,
        ...
        // Handle Ivy League Preparation service -> IVY_EXPERT role
            ...
            if (primaryIvyExpertId) {
                const primaryIvyExpert = await IvyExpert_1.default.findById(primaryIvyExpertId);
                if (!primaryIvyExpert) {
                    return res.status(404).json({
                        success: false,
            ...
            if (secondaryIvyExpertId) {
                const secondaryIvyExpert = await IvyExpert_1.default.findById(secondaryIvyExpertId);
        ...
        // Handle Education Planning service -> EDUPLAN_COACH role
            ...
            if (primaryEduplanCoachId) {
                const primaryEduplanCoach = await EduplanCoach_1.default.findById(primaryEduplanCoachId);
            ...
            if (secondaryEduplanCoachId) {
                const secondaryEduplanCoach = await EduplanCoach_1.default.findById(secondaryEduplanCoachId);
        ...
        const updatedRegistration = await StudentServiceRegistration_1.default.findById(registrationId)
            .populate('serviceId', 'name slug shortDescription icon')
            .populate('primaryOpsId')
 ...
 */
const switchActiveOps = async (req, res) => {
    try {
        const { registrationId } = req.params;
...
exports.switchActiveOps = switchActiveOps;
//# sourceMappingURL=superAdminStudentController.js.map