...
exports.getStudentAnswers = exports.submitForm = exports.removeSectionInstance = exports.addSectionInstance = exports.getServiceAnswers = exports.saveAnswer = exports.saveSectionAnswers = void 0;
 ...
 * @access  Private (Student only)
 */
const saveSectionAnswers = async (req, res) => {
    try {
        ...
        // Verify enrollment
        const enrollment = await Enrollment_1.Enrollment.findById(enrollmentId);
        ...
        let answerDoc = await Answer_1.Answer.findOne({ student: student._id });
        if (!answerDoc) {
            answerDoc = await Answer_1.Answer.create({
                student: student._id,
                answers: [],
            });
        }
        // Find or create section answer
        const sectionAnswerIndex = answerDoc.answers.findIndex((a) => a.section.toString() === sectionId &&
        ...
        answers.forEach((answer) => {
            ...
            const questionValueIndex = sectionAnswer.values.findIndex((v) => v.question.toString() === questionId);
        ...
        if (enrollment.status === Enrollment_1.EnrollmentStatus.NOT_STARTED) {
            enrollment.status = Enrollment_1.EnrollmentStatus.IN_PROGRESS;
...
exports.saveSectionAnswers = saveSectionAnswers;
const saveAnswer = async (req, res) => {
    try {
        const { enrollmentId, sectionId, sectionInstanceId, questionId, value, } = req.body;
        const userId = req.user.userId;
        ...
        while (retries > 0 && !saved) {
            try {
                ...
                if (saveError.name === 'VersionError' && retries > 1) {
                    ...
                    answerDoc = await Answer_1.Answer.findOne({ student: student._id }) || answerDoc;
 ...
 * @access  Private (Student only)
 */
const getServiceAnswers = async (req, res) => {
    try {
        ...
        const serviceSections = await ServiceSection_1.ServiceSection.find({
            service: serviceId,
            isActive: true,
        })
            .populate({
        ...
        const answerDoc = await Answer_1.Answer.findOne({ student: student._id });
        // Build response with auto-fill
        const sectionsWithAnswers = serviceSections.map((ss) => {
...
const addSectionInstance = async (req, res) => {
    try {
        ...
        const existingInstances = answerDoc.answers.filter((a) => a.section.toString() === sectionId);
        // Check maxRepeats
        ...
        // Create new instance
        const newInstanceId = (0, uuid_1.v4)();
        answerDoc.answers.push({
            section: sectionId,
...
const removeSectionInstance = async (req, res) => {
    try {
        ...
        // Remove instance
        answerDoc.answers = answerDoc.answers.filter((a) => !(a.section.toString() === sectionId &&
...
exports.removeSectionInstance = removeSectionInstance;
/**
...
const submitForm = async (req, res) => {
    try {
        ...
        // Verify enrollment
        const enrollment = await Enrollment_1.Enrollment.findById(enrollmentId).populate('service');
        if (!enrollment || enrollment.student.toString() !== student._id.toString()) {
            return res.status(403).json({
                success: false,
                message: 'Invalid enrollment or unauthorized access',
        ...
        if (answerDoc) {
            ...
            for (const sectionAnswer of answerDoc.answers) {
                for (const questionValue of sectionAnswer.values) {
                    ...
                    const hasCurrentValue = questionValue.updateHistory.some((h) => h.value === questionValue.value);
                    if (!hasCurrentValue) {
        ...
        enrollment.status = Enrollment_1.EnrollmentStatus.SUBMITTED;
